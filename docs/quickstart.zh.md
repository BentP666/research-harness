...
推荐交互式前端是 Zotero 侧边栏插件。从最新版 GitHub Release 下载
`research-harness-zotero-panel.xpi`，然后在 Zotero 中通过
**Tools → Add-ons → Install Add-on From File…** 安装。
...
偏好项、自动启动和安全边界见 [`zotero-rh-panel.md`](zotero-rh-panel.md)。旧的 Next.js 网页端已弃用，仅保留为本地实验入口。
