...
> 详细规范见：`docs/PAPER_MANAGEMENT.md`
...
3. **经验记录**方式统一（进入 provenance、GitHub Issue 或项目正式 backlog，不新增公共调试报告）
...
| `citation_sanitize` | 验证 | 按 topic 论文池清洗引用，删除幻觉引用并重编号 |
...
公共仓库只保留可复用代码、脱敏示例和精简文档。不要新增一次性调试报告、个人 handoff、私有 topic 记录或本地运行日志。

| 经验类型 | 推荐位置 | 示例 |
|----------|----------|------|
| 搜索策略 / 研究结论 | RH provenance / artifact | 自动记录 query、paper、claim、evidence |
| 缺失工具或 MCP bug | GitHub Issue 或项目 backlog | "paper_search venue_filter 不支持简写" |
| 流程改进建议 | GitHub Issue / PR discussion | "evidence gate 应要求标记矛盾观点" |
| 个人排障笔记 | 本地笔记或 agent memory | 不提交到公共仓库 |

如果某条经验已经通过测试、代码注释、README 或正式文档表达清楚，不要再额外创建重复文档。
...
2. 运行对抗审查
   adversarial_run(
       topic_id=1, artifact_id=<proposal_id>,
       proposal_snapshot={...},
       objections=[...]
   )

3. 查看结果
   adversarial_status(topic_id=1)

4. 解决并推进
   adversarial_resolve(topic_id=1, round_artifact_id=<id>, scores={...})
```
...
- **经验必须反馈**：发现的工具问题、流程改进建议进入 GitHub Issue / 正式 backlog，不新增公共调试报告
...
```
科研工作 → 发现工具/流程不足 → 开 GitHub Issue / 进入正式 backlog
                                    ↓
                              人类审阅反馈
                                    ↓
...
每次你遇到以下情况，请主动记录：
- 想用某个工具但不存在 → 开 GitHub Issue 或加入项目 backlog
- 某个工具不好用或有 bug → 开 GitHub Issue，并补 regression test
- 某个流程可以简化 → 在 PR / Issue 中讨论，必要时更新正式文档
...
```
mcp__research-harness__orchestrator_record_artifact(topic_id=1, ...)
...
api.record_artifact(topic_id=1, stage="build",
...
svc.record_artifact(topic_id=1, stage="build",
                    artifact_type="literature_map", payload={...})
```
...
| 表名 | 关键字段 |
|------|----------|
| `domains` | id, name, description, status |
| `topics` | id, name, domain_id, description, contributions, venue, deadline |
| `project_artifacts` | id, topic_id, stage, artifact_type, title, **payload_json**, version, status |
...
| `orchestrator_runs` | id, topic_id, mode, current_stage, stage_status |
| `review_issues` | id, topic_id, review_type, severity, category, summary, status |

> **历史列**：`project_id` 列在上述表中仍存在但已弃用（SQLite 不支持 DROP COLUMN）。
> 新代码只读写 `topic_id`。v0.2 起 `projects` 表虽保留在 schema，但不再被任何代码写入。
